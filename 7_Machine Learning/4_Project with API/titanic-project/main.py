...
def submit_new_profile():
    ...
    if request.method == 'POST':
        ...
        # convert in live data for prediction
        ...
        user_designed_passenger = [[age, sibsp, parch, fare, isfemale, pclass_Second, 
            pclass_Third, pclass_nan, cabin_B, cabin_C, cabin_D, cabin_E, cabin_F, cabin_G, cabin_T, cabin_Unknown, cabin_nan, embarked_Q, embarked_S, embarked_Unknown, embarked_nan, title_Master, title_Miss, title_Mr, title_Mrs, title_Rev,title_Unknown, title_nan]]
        
        ypred = model.predict_proba(user_designed_passenger)
        probability_of_surviving_character = ypred[0][1] * 100

        # send matplotlib image

        fig = plt.figure()
        objects = ('Average Survival Rate', 'Traveler')
        ypos = np.arange(len(objects))

        performance = [average_survival_rate, probability_of_surviving_character]
